...
export default function PerlinCanvas({ params, pixelRatio, paused = false }: Props) {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const rendererRef = useRef<PerlinRenderer | null>(null);
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    let renderer: PerlinRenderer;
    try {
      renderer = new PerlinRenderer(canvas);
    } catch (e) {
      console.error(e);
      return;
    }
