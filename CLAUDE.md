...
- Use **shadcn/ui** for all UI. Import from `@/components/ui/...`. Add missing pieces with `npx shadcn@latest add <component>` before building one-offs.
- Compose shadcn primitives for app-specific UI; do not replace them with bespoke controls.
- Style with **Tailwind** and theme CSS variables (`bg-background`, `text-muted-foreground`, etc.).
- Icons: **lucide-react** only.

### Page layout

- Noise Lab is a single-page fullscreen tool, not paginated content. The viewport occupies all remaining space between topbar (`h-12`) and statusbar (`h-8`); the side panel is a fixed `w-90` (360px) flex child. Do **not** wrap pages in `max-w-*` containers — the canvas needs the full width to render its previews honestly.

### Responsive

- The app must be **mobile responsive**: layouts and interactions should work well from small phones through large desktops. Use Tailwind responsive modifiers (`sm:`, `md:`, etc.) and shadcn patterns (responsive variants, stacking, overflow) so nothing is unusable on narrow viewports. shadcn/ui components are built with this in mind; still verify breakpoints when composing custom layouts.

### WebGL renderers

- WebGL renderers in `src/lib/{fireCloud,perlin,grain}.ts` are imperative-only. UI components must pass params via the `RenderFn<P>` signature in `src/types/index.ts`; never reach into the renderers to mutate WebGL state directly from JSX.
- To add a new noise type: add a renderer file, extend `NoiseKind` in `src/types/index.ts`, register it in `src/lib/download.ts`'s `exportNoise()`, and add a Canvas wrapper component following the `FireCloudCanvas` / `PerlinCanvas` / `GrainCanvas` pattern.
- Shared palette presets live in `src/lib/palettes.ts` — every renderer consumes the same library so palette behavior stays consistent across tabs.
